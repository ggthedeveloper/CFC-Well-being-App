...
// ══════════════ STATE ══════════════
let currentMood = null, chartInstance = null, chatHistory = [];
...
function doLogin() {
  ...
  if (!users[name.toLowerCase()]) { err.textContent='No account found. Please register.'; return; }
  if (users[name.toLowerCase()].hash !== hash(pass)) { err.textContent='Incorrect password.'; return; }
  err.textContent='';
...
function doRegister() {
  ...
  if (!name || !pass || !pass2) { err.textContent='Please fill in all fields.'; return; }
  if (pass.length < 4) { err.textContent='Password must be at least 4 characters.'; return; }
  if (pass !== pass2) { err.textContent='Passwords do not match.'; return; }
  const users = getUsers();
  if (users[name.toLowerCase()]) { err.textContent='Name already taken. Try another.'; return; }
  users[name.toLowerCase()] = { displayName: name, hash: hash(pass) };
  saveUsers(users);
  err.textContent='';
...
function bootApp() {
  ...
  refreshStats(); renderWeeklyHeatmap(); renderRecentHistory(); initChat();
  buildMusicTabs(); loadMusicMood(currentMood||'calm');
  buildGamesGrid();
}
...
function pickMood(mood) {
  ...
  document.getElementById('suggestionsList').innerHTML  = d.acts.map(a=>`<li class="suggestion-tag">${a}</li>`).join('');
  loadMusicMood(mood);
  buildGamesGrid();
}
...
function saveEntry() {
  ...
  const h = getHistory();
  h.push({mood:currentMood,emoji:MOODS[currentMood].emoji,note,date:new Date().toISOString()});
  saveHistory(h);
  document.getElementById('journalText').value=''; updateWordCount();
  document.querySelectorAll('.mood-btn').forEach(b=>b.classList.remove('active'));
  ...
  currentMood=null;
  refreshStats(); renderWeeklyHeatmap(); renderRecentHistory();
...
function refreshStats() {
  ...
  document.getElementById('streakBadge').innerHTML   = `🔥 ${streak}`;
  if (!h.length) { document.getElementById('statTopMood').textContent='—'; return; }
  const counts={}; h.forEach(i=>counts[i.mood]=(counts[i.mood]||0)+1);
  const top = Object.entries(counts).sort((a,b)=>b[1]-a[1])[0][0];
  document.getElementById('statTopMood').textContent = MOODS[top]?.emoji||top;
}
function calcStreak(history) {
  if (!history.length) return 0;
  const dates=[...new Set(history.map(i=>new Date(i.date).toDateString()))].map(d=>new Date(d)).sort((a,b)=>b-a);
  let streak=0,check=new Date();check.setHours(0,0,0,0);
  for(const d of dates){if(d.toDateString()===check.toDateString()){streak++;check.setDate(check.getDate()-1);}else if(d<check)break;}
  return streak;
}
...
function openModal(id) {
  ...
  if(id==='chatModal'&&!chatHistory.length) initChat();
}
...
function renderChart() {
  const h=getHistory().slice(-30),counts={};
  ...
  chartInstance=new Chart(document.getElementById('moodChart').getContext('2d'),{
    type:'doughnut',
    data:{labels,datasets:[{data:values,backgroundColor:colors.map(c=>c+'cc'),borderColor:colors,borderWidth:2}]},
    options:{plugins:{legend:{labels:{color:'#f0eaf8',font:{family:'DM Sans',size:12},padding:14}}},animation:{animateRotate:true,duration:700}}
  });
}
...
function initChat() {
  ...
  chatHistory=[];
  addAIBubble("Hi! I'm your MoodMate companion 🤖 Tell me how you're feeling, or ask me anything about wellbeing.");
}
function addUserBubble(t){const m=document.getElementById('chatMessages');m.innerHTML+=`<div class="chat-bubble bubble-user"><div class="bubble-label">YOU</div>${escapeHtml(t)}</div>`;m.scrollTop=m.scrollHeight;}
function addAIBubble(t){const m=document.getElementById('chatMessages');m.innerHTML+=`<div class="chat-bubble bubble-ai"><div class="bubble-label">MOODMATE</div>${escapeHtml(t)}</div>`;m.scrollTop=m.scrollHeight;}
function showTyping(){const m=document.getElementById('chatMessages');const d=document.createElement('div');d.className='chat-bubble bubble-ai';d.id='typingIndicator';d.innerHTML='<div class="typing-indicator"><div class="typing-dot"></div><div class="typing-dot"></div><div class="typing-dot"></div></div>';m.appendChild(d);m.scrollTop=m.scrollHeight;}
function removeTyping(){const e=document.getElementById('typingIndicator');if(e)e.remove();}

async function sendChat() {
  const input=document.getElementById('chatInput');
  const text=input.value.trim();if(!text)return;
  input.value='';addUserBubble(text);
  chatHistory.push({role:'user',content:text});showTyping();
  try {
    const h=getHistory().slice(-5);
    const ctx=h.length?`User's recent moods: ${h.map(i=>i.mood).join(', ')}.`:'';
    const res=await fetch('http://localhost:3001/chat',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({system:`You are MoodMate, a warm empathetic emotional wellness companion. Keep responses concise (2-4 sentences). Be genuine, not clinical. ${ctx}`,messages:chatHistory})});
    if(!res.ok){const e=await res.json().catch(()=>({}));throw new Error(e.error||`Error ${res.status}`);}
    const data=await res.json();removeTyping();
    const reply=data.reply||"I'm here for you.";
    chatHistory.push({role:'assistant',content:reply});addAIBubble(reply);
  } catch(err){removeTyping();addAIBubble("I'm having a connection issue. Make sure the server is running (node server.js).");console.error(err);}
}
...
function loadTrack(idx) {
  ...
  audioEl.volume=document.querySelector('.volume-slider').value;
  audioEl.addEventListener('ended',nextTrack);
  audioEl.addEventListener('error',()=>showToast('⚠️ Audio unavailable in local mode'));
  // Highlight playlist
